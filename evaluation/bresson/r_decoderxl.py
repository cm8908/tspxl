...
def myRMHSA(Q, K, V, nb_heads, r, u, v):
    '''
    Q : (bsz, 1, dim_emb)
    K,V : (bsz, klen, dim_emb)
    r : (klen, 1, dim_emb)
    u : (bsz*nb_heads, 1, d_model//nb_heads)
    '''
    bsz, klen, d_model = K.size()
    if nb_heads > 1:
        Q = Q.transpose(1,2).contiguous()
        Q = Q.view(bsz*nb_heads, d_model//nb_heads, 1)
        Q = Q.transpose(1,2).contiguous()  # (bsz*nb_heads, 1, d_model//nb_heads)
        K = K.transpose(1,2).contiguous()
        K = K.view(bsz*nb_heads, d_model//nb_heads, klen)
        K = K.transpose(1,2).contiguous()  # (bsz*nb_heads, klen, d_model//nb_heads)
        V = V.transpose(1,2).contiguous()
        V = V.view(bsz*nb_heads, d_model//nb_heads, klen)
        V = V.transpose(1,2).contiguous()
        r = r.view(klen, nb_heads, d_model//nb_heads)
        r = r.transpose(0, 1).contiguous()  # (nb_heads, klen, d_model//nb_heads)
        u = u.repeat(bsz, 1, 1)
        u = u.view(bsz*nb_heads, 1, d_model//nb_heads)
        v = v.repeat(bsz, 1, 1)
        v = v.view(bsz*nb_heads, 1, d_model//nb_heads)
    Qu = Q + u  # (bsz*nb_heads, 1, d_model//nb_heads)
    AC = torch.einsum('bid,bjd->bij', Qu, K)  # (bsz*nb_heads, 1, klen)
    Qv = Q + v  # (bsz*nb_heads, 1, d_model//nb_heads)
    BD = torch.einsum('bid,nkd->bik', Qv, r)  # (bsz*nb_heads, 1, klen)
    rel_shift(BD)
    # print(Qv.shape, v.shape); exit()
    # print(AC.shape, BD.shape); exit()

    attn_weights = (AC + BD) / Q.size(-1)**0.5  # (bsz*nb_heads, 1, klen)
    attn_weights = torch.softmax(attn_weights, dim=-1)
    attn_output = torch.bmm(attn_weights, V)  # (bsz*nb_heads, 1, d_model//nb_heads)
    # print(attn_output.shape, attn_weights.shape)
    if nb_heads > 1:
        attn_output = attn_output.transpose(1,2).contiguous()
        attn_output = attn_output.view(bsz, d_model, 1)
        attn_output = attn_output.transpose(1,2).contiguous()
        attn_weights = attn_weights.view(bsz, nb_heads, 1, klen)
...
class AutoRegressiveDecoderLayer(nn.Module):
    ...
    def __init__(self, dim_emb, nb_heads):
        ...
        self.r_net = nn.Linear(dim_emb, dim_emb, bias=False)
        self.PE = PositionalEmbedding(dim_emb)
        self.u = nn.Parameter(torch.randn(nb_heads, dim_emb//nb_heads))
        self.v = nn.Parameter(torch.randn(nb_heads, dim_emb//nb_heads))

    def reset_selfatt_keys_values(self):
        self.K_sa = None
        self.V_sa = None
        
    def forward(self, h_t, K_att, V_att, mask, segm_len):
        bsz = h_t.size(0)
        ...
        klen = self.K_sa.size(1)
        pos_seq = torch.arange(klen-1, -1, -1, device=h_t.device, dtype=h_t.dtype)
        pos_emb = self.PE(pos_seq)
        r = self.r_net(pos_emb)

        # compute self-attention between nodes in the partial tour
        t_selfattn_start = time.time()
        h_t = h_t + self.W0_selfatt( myRMHSA(q_sa, self.K_sa, self.V_sa, self.nb_heads, r, self.u, self.v)[0] ) # size(h_t)=(bsz, 1, dim_emb)
    ...
    def forward(self, h_t, K_att, V_att, mask, segm_len):
        for l in range(self.nb_layers_decoder):
            ...
            if l<self.nb_layers_decoder-1: # decoder layers with multiple heads (intermediate layers)
                h_t = self.decoder_layers[l](h_t, K_att_l, V_att_l, mask, segm_len)
...
class TSP_net(nn.Module): 
    ...
    def __init__(self, segm_len, dim_input_nodes, dim_emb, dim_ff, nb_layers_encoder, nb_layers_decoder, nb_heads, max_len_PE,
                 batchnorm=True):
    ...
    def forward(self, x, deterministic=False):
        ...
        for t in range(nb_nodes):
            ...
            prob_next_node = self.decoder(h_t, K_att_decoder, V_att_decoder, mask_visited_nodes, self.segm_len) # size(prob_next_node)=(bsz, nb_nodes+1)
...
model_train = TSP_net(args.segm_len, args.dim_input_nodes, args.dim_emb, args.dim_ff, 
              args.nb_layers_encoder, args.nb_layers_decoder, args.nb_heads, args.max_len_PE,
              batchnorm=args.batchnorm)

model_baseline = TSP_net(args.segm_len, args.dim_input_nodes, args.dim_emb, args.dim_ff, 
              args.nb_layers_encoder, args.nb_layers_decoder, args.nb_heads, args.max_len_PE,
              batchnorm=args.batchnorm)
